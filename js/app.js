...
import {unit13Questions,unit13Sections} from './booster-unit13.js';
import {unit14Questions,unit14Sections} from './booster-unit14.js';
...
async function init(){applyPreferences();bindNavigation();bindPractice();try{questions=await loadQuestions();allQuestions=[...questions,...unit1Questions,...unit2Questions,...unit3Questions,...unit4Questions,...unit5Questions,...unit6Questions,...unit7Questions,...unit8Questions,...unit9Questions,...unit10Questions,...unit11Questions,...unit12Questions,...unit13Questions,...unit14Questions];sessionQuestions=[...allQuestions];populateFilters();renderLibrary();renderBooster();renderDashboard(state,allQuestions.length);showView(location.hash.slice(1)||'dashboard');window.addEventListener('hashchange',()=>showView(location.hash.slice(1)||'dashboard'))}catch(error){document.querySelector('.main').innerHTML=`<section class="empty-view active"><h2>Question bank unavailable</h2><p>${error.message}. Make sure the data folder is published.</p></section>`}}
...
function renderBooster(){
  ...
  $('#boosterTracks13').innerHTML=unit13Sections.map(section=>`<article class="booster-track"><span>${section.count} ITEMS</span><h3>${section.track}</h3><p>${section.description}</p><button class="button primary" data-booster13-track="${section.track}">Practise ${section.track}</button></article>`).join('');
  $('#boosterTracks14').innerHTML=unit14Sections.map(section=>`<article class="booster-track"><span>${section.count} ITEMS</span><h3>${section.track}</h3><p>${section.description}</p><button class="button primary" data-booster14-track="${section.track}">Practise ${section.track}</button></article>`).join('');
  ...
  $$('[data-booster13-track]').forEach(button=>button.onclick=()=>startBooster(button.dataset.booster13Track,'13'));
  $$('[data-booster14-track]').forEach(button=>button.onclick=()=>startBooster(button.dataset.booster14Track,'14'));
  ...
  $('#startFullUnit13').onclick=()=>startBooster('ALL','13');
  $('#startFullUnit14').onclick=()=>startBooster('ALL','14');
}
function startBooster(track,unit='01'){const bank={'01':unit1Questions,'02':unit2Questions,'03':unit3Questions,'04':unit4Questions,'05':unit5Questions,'06':unit6Questions,'07':unit7Questions,'08':unit8Questions,'09':unit9Questions,'10':unit10Questions,'11':unit11Questions,'12':unit12Questions,'13':unit13Questions,'14':unit14Questions}[unit]||unit1Questions;sessionQuestions=track==='ALL'?[...bank]:bank.filter(q=>q.track===track);currentIndex=0;navigate('practice');toast(`${sessionQuestions.length} Unit ${unit} exercises loaded`)}
